...
data_queue = Queue(maxsize=1000000)
...
class World(object):
    ...
    def restart(self, args):
        ...
        for vehicle in self.world.get_blueprint_library().filter(self._actor_filter):
            if args.car == vehicle.id:
                blueprint = vehicle
        print("car selected = ", blueprint)
        # print([x.id for x in self.world.get_blueprint_library().filter(self._actor_filter)])
    ...
    def tick(self, world, clock):
        ...
        try:
            sensor_data = {
                "time": time.time(),
                "weather": {
                    "precipitation": carla.WeatherParameters().precipitation,
                    "fog": carla.WeatherParameters().fog_density,
                    "wetness": carla.WeatherParameters().wetness,
                    "precipitation": carla.WeatherParameters().precipitation,
                },
                "vehicle_details": {
                    "car_brand": "TESLA",
